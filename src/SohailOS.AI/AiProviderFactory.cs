...
public static class AiProviderFactory
{
    ...
    private static (IAiProvider, IAiCompletionProvider) CreateAuto(HttpClient httpClient)
    {
        ...
        TryAdd(providers, "openai", () => CreateOpenAi(httpClient).Item2);
    ...
    {
        try { providers.Add(new NamedProvider(name, factory())); }
    ...
    private sealed class RoutedCompletionProvider : IAiCompletionProvider, IAiProvider
    {
        ...
        public async Task<AiCompletion> CompleteAsync(string systemPrompt, string userPrompt, IReadOnlyCollection<ToolDefinition> tools, CancellationToken cancellationToken = default)
        {
            ...
            try { return await selected.Provider.CompleteAsync(systemPrompt, userPrompt, tools, cancellationToken); }
            catch when (!cancellationToken.IsCancellationRequested)
            {
                foreach (var fallback in _providers.Where(x => !ReferenceEquals(x, selected)))
                {
                    try { return await fallback.Provider.CompleteAsync(systemPrompt, userPrompt, tools, cancellationToken); }
                    catch when (!cancellationToken.IsCancellationRequested) { }
                }
                throw;
            }
        ...
        private NamedProvider Select(string prompt)
        {
            ...
            if (ContainsAny(text, "کدنویسی", "کد", "github", "debug", "c#", "python", "برنامه نویسی")) return Find("anthropic") ?? Find("openai") ?? _providers[0];
            if (ContainsAny(text, "پژوهش", "مقاله", "research", "منبع", "literature", "مرور نظام مند", "متاآنالیز")) return Find("gemini") ?? Find("openai") ?? _providers[0];
            if (ContainsAny(text, "تصویر", "چندوجهی", "multimodal", "gemini")) return Find("gemini") ?? Find("openai") ?? _providers[0];
