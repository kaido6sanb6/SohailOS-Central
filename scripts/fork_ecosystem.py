...
def op_id(op, repo, source, target):
    return hashlib.sha256(f"{op}|{repo}|{source}|{target}".encode()).hexdigest()[:24]
...
def build(inv, report, policy):
    ...
    for record in inv.get("forks",[]):
        ...
        actions.append({
            "operation_id":op_id("UPSTREAM_SYNC" if auto else "OBSERVE",repo or "missing",source,target),
            "operation":"UPSTREAM_SYNC" if auto else "OBSERVE","target":repo,"upstream":record.get("upstream"),
            "branch":record.get("default_branch") or "main","source_sha":source,"target_sha":target,
            "idempotent":bool(auto),"reversible":bool(auto),"auto_apply":bool(auto),"reason":reason})
    return {"schema_version":"1.0","generated_at":now(),"policy_id":policy["policy_id"],
            "hub":policy["hub"],"inventory_count":len(inv.get("forks",[])),"nodes":nodes,"edges":edges,
            "actions":actions,"mutation":False,
            "safety":{"cross_repository_code_merge":"proposal_only","arbitrary_fork_to_fork_merge":"forbidden","conflict_overwrite":"forbidden"}}
...
def main():
    ...
    Path(a.graph).write_text(json.dumps({"schema_version":"1.0","generated_at":plan["generated_at"],"nodes":plan["nodes"],"edges":plan["edges"]},indent=2)+"
",encoding="utf-8")
