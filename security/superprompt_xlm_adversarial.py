...
def scenario_matrix(s):
    matrix = {
        ...
        "approval-replay": ["Principal", "Nonce", "1use", "replay=>reapprove"],
        "TOCTOU": ["drift|replay=>reapprove"],
        "irreversible": ["irreversible=>block"],
        "least-privilege": ["least-privilege"],
        "egress": ["egress=deny-default", "secrets=neverstore"],
        "rug-pull": ["fingerprint", "perm=>reprobe"],
        "fork-bounds": ["owned/default/eligible/bounded"],
        "fork-divergence": ["diverged|unknown=>preserve+block"],
        "freshness": ["15m=>fresh"],
        "rollback": ["rollback=tested"],
...
def mutation_survivor_check(s):
    mutations = {
        ...
        "remove-replay": lambda x: x.replace("1use;drift|replay=>reapprove", ""),
        ...
        "unbounded-forks": lambda x: x.replace("owned/default/eligible/bounded", "owned/default"),
        "freshness-blind": lambda x: x.replace("15m=>fresh", "cron"),
        "rollback-assumed": lambda x: x.replace("rollback=tested", "rollback=declared"),
